...
from lumi.ui.player.overlay.state import move_cross_focus
...
def handle(overlay: PlayerOverlay, cmd: RoutedCommand) -> None:
    overlay._rt.state = move_cross_focus(overlay._rt.state, cmd.delta)
    overlay.activity.keyboard_activity()
    overlay.update()


register(Command.MOVE_CROSS_FOCUS, handle)
