...
from lumi.ui.player.overlay.state import move_video_focus
...
def handle(overlay: PlayerOverlay, cmd: RoutedCommand) -> None:
    overlay._rt.state = move_video_focus(overlay._rt.state, cmd.delta)
    refresh_panel(overlay)


register(Command.MOVE_VIDEO_FOCUS, handle)
